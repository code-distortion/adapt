...
class LaravelPostgreSQLSnapshot implements SnapshotInterface
{
    ...
    private static ?bool $psqlClientExists = null;

    /** @var boolean|null An internal cache of whether pg_dump exists or not. */
    private static ?bool $pgdumpExists = null;
     ...
     */
    private function ensurePsqlClientExists(): void
    {
        self::$psqlClientExists
            ??= $this->di->exec->commandRuns($this->configDTO->psqlExecutablePath . ' --version');
     ...
     */
    private function ensurePgDumpExists(): void
    {
        self::$pgdumpExists
            ??= $this->di->exec->commandRuns($this->configDTO->pgDumpExecutablePath . ' --version');

...
class BootCommandLaravel extends BootCommandAbstract
{
    ...
    private function newConfigDTO(string $connection): configDTO
    {
        ...
        return (new ConfigDTO())->projectName(config("$c.project_name"))->connection($connection)->database(config("database.connections.$connection.database"))->storageDir(rtrim(config("$c.storage_dir"), '\\/'))->snapshotPrefix('snapshot.')->databasePrefix('test_')->hashPaths($this->checkLaravelHashPaths(config("$c.look_for_changes_in")))->buildSettings(config("$c.pre_migration_imports"), config("$c.migrations"), config("$c.seeders"), false)->cacheTools(config("$c.reuse_test_dbs"), config("$c.scenario_test_dbs"))->snapshots(config("$c.snapshots.enabled"), config("$c.snapshots.take_after_migrations"), config("$c.snapshots.take_after_seeders"))->mysqlSettings(config("$c.database.mysql.executables.mysql"), config("$c.database.mysql.executables.mysqldump"))->postgresSettings(config("$c.database.pgsql.executables.psql"), config("$c.database.pgsql.executables.pg_dump"))->invalidationGraceSeconds(config("$c.invalidation_grace_seconds") ?? Settings::DEFAULT_INVALIDATION_GRACE_SECONDS);
    }
}
